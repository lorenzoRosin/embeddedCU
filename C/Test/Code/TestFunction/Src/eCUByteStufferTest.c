...
static void eCU_TEST_byteStuffBadPointer(void)
{
    ...
    /* Function */
    if( BSTF_RES_BADPOINTER == eCU_BSTF_InitCtx(NULL, la_memArea, sizeof(la_memArea)) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_InitCtx(&l_ctx, NULL, sizeof(la_memArea)) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetWherePutData(NULL, &lp_aPointer, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetWherePutData(&l_ctx, NULL, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetWherePutData(&l_ctx, &lp_aPointer, NULL) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetRemByteToGet(NULL, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetRemByteToGet(&l_ctx, NULL) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetStufChunk(NULL, la_memArea, sizeof(la_memArea), &l_varTemp32 ) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetStufChunk(&l_ctx, NULL, sizeof(la_memArea), &l_varTemp32) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_GetStufChunk(&l_ctx, la_memArea, sizeof(la_memArea), NULL) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_IsInit( NULL, &l_isInit ) )
    {
    ...
    if( BSTF_RES_BADPOINTER == eCU_BSTF_IsInit( &l_ctx, NULL ) )
    {
...
static void eCU_TEST_byteStuffBadInit(void)
{
    ...
    /* Function */
    if( BSTF_RES_NOINITLIB == eCU_BSTF_NewFrame(&l_ctx, 2u) )
    {
    ...
    if( BSTF_RES_NOINITLIB == eCU_BSTF_GetWherePutData(&l_ctx, &lp_aPointer, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_NOINITLIB == eCU_BSTF_RestartFrame(&l_ctx) )
    {
    ...
    if( BSTF_RES_NOINITLIB == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_NOINITLIB == eCU_BSTF_GetStufChunk(&l_ctx, la_memArea, sizeof(la_memArea), &l_varTemp32) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_IsInit( &l_ctx, &l_isInit ) )
    {
        if( false == l_isInit )
        {
...
static void eCU_TEST_byteStuffBadIniFrame(void)
{
    ...
    /* Function */
    if( BSTF_RES_OK == eCU_BSTF_InitCtx(&l_ctx, la_memArea, sizeof(la_memArea)) )
    {
    ...
    if( BSTF_RES_NOINITFRAME == eCU_BSTF_RestartFrame(&l_ctx) )
    {
    ...
    if( BSTF_RES_NOINITFRAME == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_NOINITFRAME == eCU_BSTF_GetStufChunk(&l_ctx, la_memArea, sizeof(la_memArea), &l_varTemp32) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_NewFrame(&l_ctx, 2u) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_RestartFrame(&l_ctx) )
    {
...
static void eCU_TEST_byteStuffBadParamEntr(void)
{
    ...
    /* Function */
    if( BSTF_RES_BADPARAM == eCU_BSTF_InitCtx(&l_ctx, la_memArea, 0u) )
    {
    ...
    if( BSTF_RES_BADPARAM == eCU_BSTF_NewFrame(&l_ctx, 0u) )
    {
    ...
    if( BSTF_RES_BADPARAM == eCU_BSTF_NewFrame(&l_ctx, (sizeof(la_memArea) +1u) ) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_NewFrame(&l_ctx, sizeof(la_memArea)) )
    {
    ...
    if( BSTF_RES_BADPARAM == eCU_BSTF_GetStufChunk(&l_ctx, la_memArea, 0u, &l_varTemp32) )
    {
...
static void eCU_TEST_byteStuffCorrupterContext(void)
{
    ...
    l_ctx.memASize = 0u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.memAFrameSize = 2u;
    l_ctx.memACtr = l_ctx.memAFrameSize + 1u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.memAFrameSize = l_ctx.memASize + 1u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.memACtr = 0u;
    l_ctx.stuffState = BSTF_SM_PRV_NEEDNEGATEPRECDATA;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.memACtr = 0u;
    l_ctx.stuffState = BSTF_SM_PRV_NEEDEOF;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.memACtr = 0u;
    l_ctx.stuffState = BSTF_SM_PRV_STUFFEND;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.stuffState = BSTF_SM_PRV_NEEDNEGATEPRECDATA;
    l_ctx.memACtr = 1u;
    la_memArea[l_ctx.memACtr - 1u] = 0xFFu;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_NewFrame(&l_ctx, 4u) )
    {
    ...
    l_ctx.memASize = 0u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_GetWherePutData(&l_ctx, &lp_aPointerP, &l_varTemp32) )
    {
    ...
    l_ctx.memASize = 0u;
    l_ctx.memAFrameSize = 1u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_RestartFrame(&l_ctx) )
    {
    ...
    l_ctx.memASize = 0u;
    l_ctx.memAFrameSize = 1u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_varTemp32) )
    {
    ...
    l_ctx.memASize = 0u;
    l_ctx.memAFrameSize = 1u;
    if( BSTF_RES_CORRUPTCTX == eCU_BSTF_GetStufChunk(&l_ctx, la_memArea, 1u, &l_varTemp32) )
    {
...
static void eCU_TEST_byteStuffOutOfMem(void)
{
    ...
    if( BSTF_RES_NOINITFRAME == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_reman) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetWherePutData(&l_ctx, &lp_aPointer, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_reman) )
    {
        if( 10u == l_reman )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[0], 1u, &l_varTemp32) )
    {
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[1], 1u, &l_varTemp32) )
    {
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[2], 1u, &l_varTemp32) )
    {
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_FRAMEENDED == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[3], 7u, &l_varTemp32) )
    {
        if( 7u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_FRAMEENDED == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[10], 1u, &l_varTemp32) )
    {
        if( 0u == l_varTemp32 )
        {
...
static void eCU_TEST_byteStuffGetRemainings(void)
{
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_varTemp32) )
    {
        if( 10u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[3], 6u, &l_varTemp32) )
    {
        if( 6u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_FRAMEENDED == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[9], 1u, &l_varTemp32) )
    {
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_NewFrame(&l_ctx, 1u) )
    {
...
static void eCU_TEST_byteStuffGeneral(void)
{
    ...
    e_eCU_BSTF_Res l_result = BSTF_RES_OK;
    uint8_t* lp_aPointer;
    ...
    if( BSTF_RES_FRAMEENDED == l_result )
    {
    ...
    while( ( BSTF_RES_OK == l_result ) && ( 0u != l_remaining ) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[l_counter], 2u, &l_varTemp32) )
    {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetRemByteToGet(&l_ctx, &l_remaining) )
    {
        if( 7u == l_remaining )
        {
...
static void eCU_TEST_byteStuffGeneralAnother(void)
{
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[l_counter], 1u, &l_varTemp32) )
    {
        l_counter += l_varTemp32;
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[l_counter], 7u, &l_varTemp32) )
    {
        l_counter += l_varTemp32;
        if( 7u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_FRAMEENDED == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[l_counter], 7u, &l_varTemp32) )
    {
        l_counter += l_varTemp32;
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_OK == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[0u], 1u, &l_varTemp32) )
    {
        if( 1u == l_varTemp32 )
        {
    ...
    if( BSTF_RES_FRAMEENDED == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[0u], 1u, &l_varTemp32) )
    {
        if( 1u == l_varTemp32 )
        {
...
static void eCU_TEST_byteStuffCorrecteness(void)
{
    ...
    if( BSTF_RES_FRAMEENDED == eCU_BSTF_GetStufChunk(&l_ctx, &la_memAreaFinalChunk[0], sizeof(la_memAreaFinalChunk), &l_varTemp32) )
    {
        if( 9u == l_varTemp32 )
        {
